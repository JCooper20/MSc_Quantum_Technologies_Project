"""
scripts/compare_stage6_ppo_L24_k8.py
--------------------------------------
Compares PPO and REINFORCE for L=24, k=8:

  A. REINFORCE:  R = S_final(L/2)          (episode reward)
  B. PPO:        clipped surrogate + GAE    (ε=0.2, γ=0.99, λ=0.95)
...
Loads existing checkpoint for variant A from
  results/checkpoints/policy_L24_k8.npz       (REINFORCE)
  results/checkpoints/policy_ppo_L24_k8.npz   (PPO policy head)
...
Saves to results/figures/stage6_compare_ppo_L24_k8_*.png/.pdf
"""
...
from src.training.ppo import train_policy_ppo
...
def main():
    ...
    ckpt_b       = 'results/checkpoints/policy_ppo_L24_k8.npz'
    ckpt_b_v     = 'results/checkpoints/policy_ppo_L24_k8_vparams.npz'

    t0 = time.time()

    # ── Variant A: REINFORCE ─────────────────────────────────────────────────
    if os.path.exists(ckpt_a):
        print(f"Variant A: loading REINFORCE params from {ckpt_a}")
        ...
        print("Variant A: training (REINFORCE)...")
        params_a, history_a = train_policy(
            L=L, depth=depth, k_per_layer=k,
            n_batches=150, batch_size=48, lr=3e-4,
            pretrain_episodes=500, pretrain_epochs=30,
    ...
    if os.path.exists(ckpt_b):
        print(f"\nVariant B: loading PPO params from {ckpt_b}")
        params_b = load_params(ckpt_b)
        print("Variant B: re-training to capture history...")
        params_b, vparams_b, history_b = train_policy_ppo(
            L=L, depth=depth, k_per_layer=k,
            n_batches=150, batch_size=48, lr=3e-4,
            pretrain_episodes=500, pretrain_epochs=30,
        )
    else:
        print("\nVariant B: training (PPO)...")
        params_b, vparams_b, history_b = train_policy_ppo(
            L=L, depth=depth, k_per_layer=k,
            n_batches=150, batch_size=48, lr=3e-4,
            pretrain_episodes=500, pretrain_epochs=30,
        ...
        np.savez(ckpt_b_v, **{k_: np.array(v) for k_, v in vparams_b.items()})
        print(f"Params saved → {ckpt_b}")
        print(f"Value params saved → {ckpt_b_v}")
    ...
    ax.plot(batches, history_a['entropy'], lw=1.5,
            label='REINFORCE (A)', color='C0')
    ax.plot(batches, history_b['entropy'], lw=1.5,
            label='PPO (B)', color='C1', ls='--')
    ax.set_xlabel('Batch number')
    ax.set_ylabel(r'$\langle S(L/2) \rangle$')
    ax.set_title(r'Training curves — $L=24$, $k=8$: REINFORCE vs PPO')
    ax.legend()
    fig.tight_layout()
    fig.savefig('results/figures/stage6_compare_ppo_L24_k8_training.png', dpi=200)
    fig.savefig('results/figures/stage6_compare_ppo_L24_k8_training.pdf')
    print("\nSaved → stage6_compare_ppo_L24_k8_training.png/.pdf")

    # ── Figure 2: entropy trajectories ───────────────────────────────────────
    print("\nCollecting trajectories for variant A (REINFORCE)...")
    (a_mean, a_sem), (b_mean, b_sem), (u_mean, u_sem) = collect_trajectories(
        params_a, L, depth, k, n_episodes)

    print("Collecting trajectories for variant B (PPO)...")
    ...
    for mean, sem, label, color, ls in [
        (a_mean,  a_sem,  'Adaptive A (REINFORCE)',    'C0', '-'),
        (ba_mean, ba_sem, 'Adaptive B (PPO)',           'C1', '--'),
    ...
    ax.set_title(r'Entropy trajectory — $L=24$, $k=8$: REINFORCE vs PPO')
    ax.legend()
    fig.tight_layout()
    fig.savefig('results/figures/stage6_compare_ppo_L24_k8_trajectory.png', dpi=200)
    fig.savefig('results/figures/stage6_compare_ppo_L24_k8_trajectory.pdf')
    print("Saved → stage6_compare_ppo_L24_k8_trajectory.png/.pdf")

    print(f"\nTotal time: {(time.time()-t0)/60:.1f} min.")
    print(f"\nFinal S(L/2):")
    print(f"  Variant A (REINFORCE): {a_mean[-10:].mean():.3f}")
    print(f"  Variant B (PPO):       {ba_mean[-10:].mean():.3f}")
