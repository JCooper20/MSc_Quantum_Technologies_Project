"""
scripts/compare_stage6_stepwise_L24_k8.py
------------------------------------------
Compares two REINFORCE variants for L=24, k=8:

  A. Episode reward:  R = S_final(L/2)               (original)
  B. Per-step reward: R_t = S_t(L/2) - S_{t-1}(L/2)  (stepwise)

Both use identical hyperparameters and supervised pre-training.
Loads existing checkpoint for variant A from results/checkpoints/policy_L24_k8.npz
if available; always retrains variant B.
...
Saves to results/figures/stage6_compare_stepwise_L24_k8_*.png/.pdf
"""
...
from src.training.reinforce import train_policy, train_policy_stepwise
...
def collect_trajectories(params, L, depth, k, n_episodes=100):
    """Run n_episodes of adaptive + boundary + uncontrolled, return per-layer arrays."""
...
def main():
    ...
    ckpt_b       = 'results/checkpoints/policy_L24_k8_stepwise.npz'

    t0 = time.time()

    # ── Variant A: episode reward ────────────────────────────────────────────
    if os.path.exists(ckpt_a):
        print(f"Variant A: loading from {ckpt_a}")
        params_a = load_params(ckpt_a)
        # Re-run training for history (needed for training curve)
        ...
        print(f"Variant A: training (episode reward)...")
        params_a, history_a = train_policy(
            L=L, depth=depth, k_per_layer=k,
            n_batches=150, batch_size=48, lr=3e-4,
            pretrain_episodes=500, pretrain_epochs=30,
    ...
    # ── Variant B: per-step reward ───────────────────────────────────────────
    t1 = time.time()
    print(f"\nVariant B: training (per-step reward)...")
    params_b, history_b = train_policy_stepwise(
        L=L, depth=depth, k_per_layer=k,
        n_batches=150, batch_size=48, lr=3e-4,
        pretrain_episodes=500, pretrain_epochs=30,
    )
    ...
    ax.plot(batches, history_a['entropy'], lw=1.5, label='Episode reward (A)', color='C0')
    ax.plot(batches, history_b['entropy'], lw=1.5, label='Per-step reward (B)', color='C1', ls='--')
    ax.set_xlabel('Batch number')
    ax.set_ylabel(r'$\langle S(L/2) \rangle$')
    ax.set_title(r'Training curves — $L=24$, $k=8$: episode vs per-step reward')
    ax.legend()
    fig.tight_layout()
    fig.savefig('results/figures/stage6_compare_stepwise_L24_k8_training.png', dpi=200)
    fig.savefig('results/figures/stage6_compare_stepwise_L24_k8_training.pdf')
    print("\nSaved → stage6_compare_stepwise_L24_k8_training.png/.pdf")

    # ── Figure 2: entropy trajectories ───────────────────────────────────────
    print("\nCollecting trajectories for variant A...")
    (a_mean, a_sem), (b_mean, b_sem), (u_mean, u_sem) = collect_trajectories(
        params_a, L, depth, k, n_episodes)

    print("Collecting trajectories for variant B...")
    (ba_mean, ba_sem), _, _ = collect_trajectories(
        params_b, L, depth, k, n_episodes)

    layers = np.arange(1, depth + 1)
    fig, ax = plt.subplots(figsize=(9, 5))

    for mean, sem, label, color, ls in [
        (a_mean,  a_sem,  'Adaptive A (episode reward)',   'C0', '-'),
        (ba_mean, ba_sem, 'Adaptive B (per-step reward)',  'C1', '--'),
    ...
    ax.set_title(r'Entropy trajectory — $L=24$, $k=8$: episode vs per-step reward')
    ax.legend()
    fig.tight_layout()
    fig.savefig('results/figures/stage6_compare_stepwise_L24_k8_trajectory.png', dpi=200)
    fig.savefig('results/figures/stage6_compare_stepwise_L24_k8_trajectory.pdf')
    print("Saved → stage6_compare_stepwise_L24_k8_trajectory.png/.pdf")

    print(f"\nTotal time: {(time.time()-t0)/60:.1f} min.")
    print(f"\nFinal S(L/2):")
    print(f"  Variant A (episode): {a_mean[-10:].mean():.3f}")
    print(f"  Variant B (stepwise): {ba_mean[-10:].mean():.3f}")
