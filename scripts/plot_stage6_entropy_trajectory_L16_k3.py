"""
scripts/plot_stage6_entropy_trajectory_L16_k3.py
-------------------------------------------------
Retrains REINFORCE policy for L=16, k=3, saves params to
results/checkpoints/policy_L16_k3.npz, then runs 100 episodes
for four conditions recording S(L/2) after every circuit layer.

Conditions:
  1. Trained REINFORCE policy (adaptive)
  2. Boundary-avoiding heuristic
  3. Random measurements at rate k/L = 3/16 ≈ 0.19
  4. Uncontrolled circuit (p_m = 0, no measurements)

Plots mean ± SEM trajectories on one figure and saves to
results/figures/stage6_entropy_trajectory_L16_k3.png/.pdf
"""
...
def main():
    ...
    # ── Save params ──────────────────────────────────────────────────────────
    ckpt_path = 'results/checkpoints/policy_L16_k3.npz'
    np.savez(ckpt_path, **{k_: np.array(v) for k_, v in params.items()})
    print(f"Params saved → {ckpt_path}")

    # ── Phase 2: 100-episode evaluation with per-layer entropy ───────────────
    ...
    def stats(trajs):
        a = np.array(trajs)   # (n_episodes, depth)
    ...
    ax.set_title(r'Entropy trajectory — $L=16$, $k=3$, 100 episodes')
    ax.legend()
    fig.tight_layout()
    fig.savefig('results/figures/stage6_entropy_trajectory_L16_k3.png', dpi=200)
    fig.savefig('results/figures/stage6_entropy_trajectory_L16_k3.pdf')
    print("\nSaved → results/figures/stage6_entropy_trajectory_L16_k3.png/.pdf")
