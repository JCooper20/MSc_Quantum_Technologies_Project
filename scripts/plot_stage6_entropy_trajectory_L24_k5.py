"""
scripts/plot_stage6_entropy_trajectory_L24_k5.py
-------------------------------------------------
Loads trained policy for L=24, k=5 from results/checkpoints/policy_L24_k5.npz
...
Conditions:
  ...
  3. Random measurements at rate k/L = 5/24 ≈ 0.21
  4. Uncontrolled circuit (p_m = 0, no measurements)

Plots mean ± SEM trajectories on one figure and saves to
results/figures/stage6_entropy_trajectory_L24_k5.png/.pdf
"""
...
def main():
    L, k, depth = 24, 5, 96
    n_episodes   = 100
    ckpt_path    = 'results/checkpoints/policy_L24_k5.npz'
    ...
    ax.set_title(r'Entropy trajectory — $L=24$, $k=5$, 100 episodes')
    ax.legend()
    fig.tight_layout()
    fig.savefig('results/figures/stage6_entropy_trajectory_L24_k5.png', dpi=200)
    fig.savefig('results/figures/stage6_entropy_trajectory_L24_k5.pdf')
    print("\nSaved → results/figures/stage6_entropy_trajectory_L24_k5.png/.pdf")
