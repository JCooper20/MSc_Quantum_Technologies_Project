"""
scripts/plot_stage6_entropy_trajectory_L32_k12.py
--------------------------------------------------
Loads trained policy for L=32, k=12 from results/checkpoints/policy_L32_k12.npz
...
Conditions:
  ...
  3. Random measurements at rate k/L = 12/32 = 0.375
  4. Uncontrolled circuit (p_m = 0, no measurements)

Plots mean ± SEM trajectories on one figure and saves to
results/figures/stage6_entropy_trajectory_L32_k12.png/.pdf
"""
...
def main():
    L, k, depth = 32, 12, 128
    n_episodes   = 100
    ckpt_path    = 'results/checkpoints/policy_L32_k12.npz'
    ...
    ax.set_title(r'Entropy trajectory — $L=32$, $k=12$, 100 episodes')
    ax.legend()
    fig.tight_layout()
    fig.savefig('results/figures/stage6_entropy_trajectory_L32_k12.png', dpi=200)
    fig.savefig('results/figures/stage6_entropy_trajectory_L32_k12.pdf')
    print("\nSaved → results/figures/stage6_entropy_trajectory_L32_k12.png/.pdf")
