"""
Steady-state check for the fully-connected geometry.

Runs the Phase A fully-connected simulator (run_trajectory_fc) out to a
deep circuit and plots S(L/2) vs depth, averaged over n_traj
trajectories. 
"""
...
def mean_entropy_history(L, depth, p_m, n_traj):
    ...
    """
    runs = [run_trajectory_fc(L, depth, p_m)['entropy_history']
...
def main():
    ...
    fig.suptitle("Steady-state check — fully connected "
                 "(dashed = main-sweep depth 4N)")
    fig.tight_layout()
    fig.savefig("results/figures/steady_state_fc.png", dpi=200)
    plt.close(fig)
    print("Saved results/figures/steady_state_fc.png")
