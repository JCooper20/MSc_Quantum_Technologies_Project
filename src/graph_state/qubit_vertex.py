"""
Holds the vertex operator (local Clifford, as a VOP code) and the set of
neighbouring qubits (graph edges) for one qubit. VOP updates compose via
the Clifford multiplication table.
"""
# Imports
...
class QubitVertex(object):
  ...
  def __str__(self):
    return 'VOP: {}, Neighbors: {}'.format(self.vop_code, self.neighbors)
